...
func DataFileFromParquet(path string, size int64, schema *Schema, r io.ReaderAt) (DataFile, *Schema, error) {
	...
	bldr, err := NewDataFileBuilder(
		NewPartitionSpec(),
		EntryContentData,
		path,
		ParquetFile,
		map[int]any{}, // TODO: At present Parquet writes are assumed to be unpartitioned.
		f.NumRows(),
		size,
	)
	if err != nil {
	...
	bldr.ColumnSizes(colSizes(f))
	bldr.LowerBoundValues(lower)
	bldr.UpperBoundValues(upper)
